...
export function ProjectSelector({ compact = false }: { compact?: boolean }) {
  ...
  return <div className={cn("relative", compact ? "w-full" : "min-w-[230px]")}>
    <select aria-label="Current project" value={project.id} onChange={event => setProjectId(event.target.value)} className={cn("w-full appearance-none border border-border/70 bg-card/70 pr-8 text-foreground outline-none backdrop-blur-md focus:ring-2 focus:ring-ring", compact ? "h-9 rounded-lg px-2.5 text-[11px]" : "h-10 rounded-xl px-3 text-xs font-semibold")}>
