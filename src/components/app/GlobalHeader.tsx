...
export function GlobalHeader({ track }: { track: Track }) {
  ...
  return <>
    <header className="odyssey-header relative z-[70] mx-3 mt-3 flex h-[62px] shrink-0 items-center justify-between px-5 lg:mx-5 lg:px-7">
      <Link to={config.dashboard} aria-label="Euclid dashboard"><EuclidWordmark/></Link>
      <nav className="absolute left-1/2 hidden -translate-x-1/2 items-center gap-1 md:flex" aria-label="Primary navigation">{navItems.map(item=><Link key={item.id} to={item.to} className={cn("rounded-full px-4 py-2 text-xs font-medium text-muted-foreground transition-colors hover:bg-card/70 hover:text-foreground",section===item.id&&"bg-card text-foreground shadow-sm")}>{item.label}</Link>)}</nav>
      <div className="flex items-center gap-1">
        <Button variant="ghost" size="icon" className={cn("relative h-9 w-9",messengerOpen&&"bg-card shadow-sm")} aria-label="Messages" onClick={()=>{setMessengerOpen(v=>!v);setNotificationsOpen(false);setProfileOpen(false)}}><MessageSquare size={16}/><span className="absolute right-1 top-0 rounded-full bg-destructive px-1 text-[8px] font-bold text-destructive-foreground">3</span></Button>
        <div className="relative"><Button variant="ghost" size="icon" className={cn("relative h-9 w-9",notificationsOpen&&"bg-card shadow-sm")} aria-label="Notifications" onClick={()=>{setNotificationsOpen(v=>!v);setProfileOpen(false);setMessengerOpen(false)}}><Bell size={16}/><span className="absolute right-1 top-0 rounded-full bg-primary px-1 text-[8px] font-bold text-primary-foreground">3</span></Button>
