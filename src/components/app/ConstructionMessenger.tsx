...
import { ArrowUpRight, Hash, Paperclip, Pin, Plus, Search, Smile, Users, X } from "lucide-react";
...
/** Compact header dropdown — conversation previews plus a link to the full messages page. */
export function MessengerDropdown({ open, onClose }: { open: boolean; onClose: () => void }) {
  const [search, setSearch] = useState("");
  const filtered = useMemo(() => channels.filter(c => `${c.name} ${c.preview}`.toLowerCase().includes(search.toLowerCase())), [search]);
  if (!open) return null;
  return (
    <div className="odyssey-popover fixed right-[4%] top-[84px] z-[100] flex max-h-[calc(100vh-104px)] w-[min(380px,calc(100vw-2rem))] flex-col overflow-hidden lg:right-[9%]">
      <div className="flex items-center justify-between px-4 py-4">
        <div className="flex items-center gap-2">
          <p className="font-display text-base font-bold">Messages</p>
          <span className="rounded-full bg-destructive px-1.5 py-0.5 text-[9px] font-bold text-destructive-foreground">3</span>
        </div>
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="icon" className="h-8 w-8"><Plus size={15} /></Button>
          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={onClose}><X size={14} /></Button>
        </div>
      </div>
      <div className="mx-4 flex items-center gap-2 rounded-full border border-border/60 px-3 py-1.5">
        <Search size={13} className="text-muted-foreground" />
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search messages..." className="min-w-0 flex-1 bg-transparent text-[11px] outline-none" />
      </div>
      <div className="mt-2 overflow-y-auto">
        {["Pinned", "Project channels", "Channels"].map(section => {
          const rows = filtered.filter(c => c.section === section);
          if (!rows.length) return null;
          return (
            <div key={section}>
              <p className="px-4 pb-1 pt-3 text-[9px] font-bold uppercase text-muted-foreground">{section}</p>
              {rows.map(c => (
                <Link key={c.name} to={`/messages?channel=${c.name}`} onClick={onClose}
                  className="flex w-full items-center gap-3 border-b border-border/45 px-4 py-3 text-left hover:bg-card/55">
                  <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary"><Hash size={14} /></span>
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-xs font-semibold">{c.name}</span>
                    <span className="mt-0.5 block truncate text-[10px] text-muted-foreground">{c.preview}</span>
                  </span>
                  {c.unread > 0 && <span className="shrink-0 rounded-full bg-primary px-1.5 py-0.5 text-[9px] font-bold text-primary-foreground">{c.unread}</span>}
                </Link>
              ))}
            </div>
          );
      ...
      <Link to="/messages" onClick={onClose} className="flex items-center justify-center gap-1 border-t border-border/60 px-4 py-3 text-[10px] font-semibold text-primary">
        Open all messages <ArrowUpRight size={12} />
      </Link>
    </div>
  );
}

/** Full three-pane messenger surface used by the expanded messages page. */
export function MessengerBody({ initialChannel, onClose }: { initialChannel?: string; onClose?: () => void }) {
  const [activeChannel, setActiveChannel] = useState(initialChannel ?? channels[0].name);
  const [search, setSearch] = useState("");
