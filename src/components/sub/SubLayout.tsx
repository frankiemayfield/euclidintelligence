...
export function SubLayout({ children }: { children: React.ReactNode }) {
  ...
  const getSection = (): GlobalSection => {
    ...
    if (/^\/sub\/(active|schedule|time)/.test(location.pathname)) return "dashboard";
  ...
  return (
    <div className="odyssey-app relative flex h-screen flex-col overflow-hidden">
      ...
      <div className="relative z-10 flex flex-1 gap-3 overflow-hidden px-3 pb-3 pt-3 lg:gap-4 lg:px-5 lg:pb-5">
        ...
        {/* Main + Euclid */}
        <main data-dense-workspace={location.pathname.includes("scope-analyzer") || location.pathname.includes("estimate-builder") || location.pathname.includes("bid-leveling") || location.pathname.includes("market-comparison") || location.pathname.includes("estimate-comparison") ? "true" : undefined} className="min-w-0 flex-1 overflow-y-auto odyssey-scroll-fade rounded-2xl">{children}</main>
